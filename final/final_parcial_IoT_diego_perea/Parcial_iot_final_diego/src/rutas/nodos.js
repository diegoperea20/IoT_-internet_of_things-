            ...
            tempConn.query('SELECT * FROM nodo', function (error, result) {
                ...
                if (error) {
                    ...
                    for (i = 0; i < resultado.length; i++) { 		//se lee el resultado y se arma el json
                        json1 = { "idnodo": resultado[i].idnodo, "nombre": resultado[i].nombre, };
...
router.post('/nodos', (req, res) => {
    ...
    connection.getConnection(function (error, tempConn) { //conexion a mysql
        if (error) {
            ...
            console.log('Conexion correcta.');
            tempConn.query('INSERT INTO nodo VALUES(?,?)', [json1.idnodo, json1.nombre], function (error, result) { //se ejecuta lainserción
                if (error) {
...
router.put('/nodos/:idnodo', (req, res) => {
    ...
    connection.getConnection(function (error, tempConn) { //conexion a mysql
        ...
        if (error) {
            ...
            console.log('Conexion correcta.');
            tempConn.query('UPDATE nodo SET nombre=? WHERE idnodo=?', [json1.nombre, id], function (error, result) { //se ejecuta lainserción
                if (error) {
                    res.send("error al ejecutar el query");
                } else {
                    tempConn.release();
